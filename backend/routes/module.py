...
from models import Module, CourseModule, UserModule, ModulePost
...
async def get_all_modules(
    ...
    """
    temp_query = select(Module) # build the query with no parameters, doesn't touch db yet
    ...
    if current_user.role.name == "admin":
     ...
     # used ai with the prompt "sql alchemy .contains like case insensitive query param" and found ilike()
                )
            )   

        # allows filtering by user_id in module (I think should work, more for future proofing since hard to test now)
        ...
        temp_query = temp_query.join(UserModule).where(UserModule.user_id == current_user.id)
