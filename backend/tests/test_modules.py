...
@pytest.mark.asyncio
async def test_get_modules_success_as_admin_returns_list(
...
async def test_get_modules_invalid_422(client: AsyncClient, auth_headers):
    ...
    resp = await client.get("/api/modules?course_id=abc", headers=auth_headers)
...
@pytest.mark.asyncio
async def test_get_module_forbidden_for_non_associated_user(
    client: AsyncClient, auth_headers, user_headers
):
...
async def test_create_module_requires_auth(client: AsyncClient):
    ...
    resp = await client.post("/api/modules", json={"module_data": {"title": "x"}})
    assert resp.status_code == 401
...
async def test_create_module_forbidden_for_non_admin(client: AsyncClient, user_headers):
    ...
    resp = await client.post(
        "/api/modules", json={"module_data": {"title": "x"}}, headers=user_headers
    )
...
async def test_create_module_invalid_body_422(client: AsyncClient, auth_headers):
    ...
    resp = await client.post("/api/modules", json={}, headers=auth_headers)
    assert resp.status_code == 422
...
async def test_update_module_requires_auth(client: AsyncClient):
    ...
    resp = await client.patch(
        "/api/modules/1", json={"module_data": {"title": "Updated"}}
    )
    assert resp.status_code == 401
...
@pytest.mark.asyncio
async def test_delete_module_forbidden_for_non_admin(
    client: AsyncClient, auth_headers, user_headers
):
    ...
    resp = await client.delete(f"/api/modules/{module_id}", headers=user_headers)
    assert resp.status_code == 403
...
@pytest.mark.asyncio
async def test_delete_module_success_admin(
    client: AsyncClient, auth_headers, user_headers
):
    ...
    resp = await client.delete(f"/api/modules/{module_id}", headers=auth_headers)
    assert resp.status_code == 204
    assert resp.text == ""

    get_resp = await client.get(f"/api/modules/{module_id}", headers=user_headers)
